...
struct MushafFlowView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UITextView {
        ...
        textView.tintColor = UIColor(named: "BrownText")
    ...
    private func makeAttributedText() -> (text: NSAttributedString, ayahRanges: [Int: NSRange]) {
        ...
        let collectedColor = bronzeColor.withAlphaComponent(0.14)
        ...
        for (index, verse) in verses.enumerated() {
            ...
            let markerText = "﴿\(Self.arabicIndicNumber(verse.ayahInSurah))﴾ "
            let location = result.length
            var shared: [NSAttributedString.Key: Any] = [
                .font: bodyFont,
            ...
            if verse.globalAyah == currentGlobalAyah {
                shared[.backgroundColor] = highlightColor
            } else if collectedAyahs.contains(verse.globalAyah) {
                shared[.backgroundColor] = collectedColor
            }
            var bodyAttributes = shared
            ...
            if let background = shared[.backgroundColor] {
                markerAttributes[.backgroundColor] = background
            }
